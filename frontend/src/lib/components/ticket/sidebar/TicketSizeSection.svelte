...
<details class="group rounded-lg border border-gray-200 dark:border-gray-700" open>
	...
	<div class="space-y-2 border-t border-gray-100 dark:border-gray-700 p-3">
		<select value={settings.type} onchange={handleTypeChange} class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-1.5 text-xs" aria-label="Ticket type">
		...
		{#if settings.type === 'others'}
			<div class="flex gap-2">
				<div class="flex-1">
					<label for="ticket-width" class="mb-1 block text-xs text-gray-500 dark:text-gray-400">Width (mm)</label>
					<input id="ticket-width" type="number" value={settings.width} onchange={handleWidth} step="0.1" min="10" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-1 text-xs" />
				</div>
				<div class="flex-1">
					<label for="ticket-height" class="mb-1 block text-xs text-gray-500 dark:text-gray-400">Height (mm)</label>
					<input id="ticket-height" type="number" value={settings.height} onchange={handleHeight} step="0.1" min="10" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-1 text-xs" />
				</div>
			</div>
		{/if}

		<p class="text-xs text-gray-400 dark:text-gray-500">{settings.width.toFixed(1)} x {settings.height.toFixed(1)} mm</p>
	</div>
</details>
